...
export default function LoginPage() {
  ...
  const [stage, setStage] = useState('password'); // 'password', 'otp', 'resetPassword', 'success'
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const initiateResponse = await cognitoClient.send(new InitiateAuthCommand({
        AuthFlow: 'CUSTOM_AUTH',
        ClientId: clientId,
        AuthParameters: { USERNAME: username },
      }));
      const challenge = initiateResponse.ChallengeParameters?.challenge;
      if (initiateResponse.ChallengeName !== 'CUSTOM_CHALLENGE' || challenge !== 'PASSWORD_CHALLENGE') {
        throw new Error(initiateResponse.ChallengeParameters?.failureReason || `Expected PASSWORD_CHALLENGE but got ${challenge || 'none'}`);
      }
      ...
      if (challengeResponse.ChallengeName === 'CUSTOM_CHALLENGE' && nextChallenge === 'OTP_CHALLENGE') {
        ...
        throw new Error(challengeResponse.ChallengeParameters?.failureReason || `Expected OTP_CHALLENGE after password but got ${nextChallenge || 'none'}`);
      }
    } catch (err) {
    ...
    try {
      ...
      if (challengeResponse.AuthenticationResult) {
        ...
        if (nextChallenge === 'OTP_CHALLENGE') {
          ...
          throw new Error(challengeResponse.ChallengeParameters?.failureReason || `Unexpected challenge: ${nextChallenge || 'none'}`);
        }
      } else {
